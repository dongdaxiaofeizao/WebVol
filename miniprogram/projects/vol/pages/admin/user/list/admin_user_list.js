...
Page({
	...
	bindScoreTap: async function (e) {
		let curIdx = pageHelper.dataset(e, 'idx');
		let scoreType = pageHelper.dataset(e, 'type');
	...
	bindScoreCmpt: async function (e) {
		if (!AdminBiz.isAdmin(this)) return;
		...
		let score = Math.abs(Number(this.data.formScoreCnt.trim()));
		if (!score) return pageHelper.showModal('积分不能为空或者小于等于0');

		let desc = this.data.formScoreDesc.trim();
		if (!desc) return pageHelper.showModal('请填写备注');
		...
		try {
			await cloudHelper.callCloudSumbit('admin/score_change', params).then(res => {
				let total = Number(this.data.dataList.list[idx].USER_SCORE_TOTAL);
				let used = Number(this.data.dataList.list[idx].USER_SCORE_USED);
				if (scoreType == '增加')
					total += score;
				else
					used += score;
				...
				this.setData({
					['dataList.list[' + idx + '].USER_SCORE_TOTAL']: total,
					['dataList.list[' + idx + '].USER_SCORE_USED']: used,
					scoreModalShow: false,
					formScoreCnt: '',
					curIdx: -1,
					scoreType: '增加',
				...
				pageHelper.showSuccToast('积分' + scoreType + '成功');
			});
