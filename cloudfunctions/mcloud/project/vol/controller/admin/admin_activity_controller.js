...
class AdminActivityController extends BaseProjectAdminController {
	...
	async getAdminActivityList() {
		...
		for (let k = 0; k < list.length; k++) {
			...
			list[k].ACTIVITY_BEGIN = timeUtil.timestamp2Time(list[k].ACTIVITY_BEGIN, 'Y-M-D h:m');
	...
	async getActivityDetail() {
		...
		if (activity) {
			...
			activity.ACTIVITY_BEGIN = timeUtil.timestamp2Time(activity.ACTIVITY_BEGIN, 'Y-M-D h:m');
		}
	...
	async insertActivity() {
		...
		let rules = {
			...
			start: 'must|string|name=活动时间',

			begin: 'must|string|name=报名开始时间',
