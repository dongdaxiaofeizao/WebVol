...
class ActivityController extends BaseProjectController {
	...
	async getActivityList(type) {
		...
		for (let k = 0; k < list.length; k++) {
			...
			if (type == 'run')
				...
				list[k].start = timeUtil.timestamp2Time(list[k].ACTIVITY_START, 'Y-M-D- h:m') + ' ' + week;
	...
	async viewActivity() {
		...
		if (activity) {
			activity.begin = timeUtil.timestamp2Time(activity.ACTIVITY_BEGIN, 'Y-M-D h:m');
	...
	async getMyActivityJoinList() {
		...
		for (let k = 0; k < list.length; k++) {
			list[k].time = this._getTimeShow(list[k].activity.ACTIVITY_START);
	...
	async getMyActivityJoinDetail() {
		...
		if (activityJoin) {
			...
			activityJoin.time = this._getTimeShow(activityJoin.activity.ACTIVITY_START);
		}
		return activityJoin;
